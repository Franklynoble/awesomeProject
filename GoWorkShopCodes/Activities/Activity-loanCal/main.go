...
func main() {
	item1 :=	salesTaxCal(0.99,7.5)
	item2 := salesTaxCal(2.75,1.5)
	item3 := salesTaxCal(0.87,2)

	allItems := item1+ item2 + item3
	fmt.Println("Sales Tax Total: ",allItems)

	//loanCal(350, 1000, 10000, 12, 47)
...
func loanCal(creditScore, monthlyIncome, loanAmount, loanTerm, interestRate int) {
	...
	if creditScore >= 450 && interestRate >= goodCreditInterestRate && loanTerm%12 == 0 && percentGoodCreditScore <= monthlyPayment {
	...
	if 450 < creditScore && interestRate == lessThanInterestRate && lessThanGoodCreditScoreMonthlyIncomePercent <= monthlyIncome && loanTerm%12 == 0 && percentLessthanGoodCredit <= monthlyPayment {
		totalCost = loanAmount * interestRate * loanTerm
		application1()
		Approved = false
		fmt.Println(CreditScore, creditScore, Income, monthlyIncome, LoanAmount, loanAmount, MonthlyPayment, monthlyPayment, Rate, interestRate, TotalCost, totalCost, Approved)
	} else
	if 0 <= creditScore || 0 <= interestRate || 0 <= monthlyIncome || 0 <= loanAmount {
...
func taxCal(cost float64 , salesTaxtRate []float64) float64 {
	var totalTax float64
	var final float64
	for i := 0 ; i < len(salesTaxtRate) ; i ++ {

		totalTax = salesTaxtRate[i]
		final =  cost * totalTax/100
	}
...
func salesTaxCal(cost float64, taxRate float64) float64 {
